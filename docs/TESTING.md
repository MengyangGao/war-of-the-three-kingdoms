...
2. 93 项规则断言：卡牌、技能、牌区事件、判定时序、动作校验、检查点恢复、事件信封、时间线和身份平衡反例。
3. 濒死专项测试。
4. 60 局固定种子的 8 人 AI 对战，检查异常、胜负结束、事件 schema 和 130 张实体牌守恒。
5. 3–8 人各 200 局平衡矩阵，检查身份胜率下限、主阵营支配率和回合 P95。
...
npm run test:balance   # 3–8 人各 200 局身份平衡矩阵
...
- 修改 AI、角色表、初始手牌或伤害/救援逻辑时必须运行 `npm run test:balance`。
- 修改快照字段时必须保留“原局与恢复局最终牌区完全一致”回归。
- 新战报必须经过 schema 2 信封，角色互动不得依赖中文文本解析。
