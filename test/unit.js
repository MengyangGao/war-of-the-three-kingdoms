...
async function main() {
  ...
  /* 26. structured events and serializable state boundary */
  {
    ...
    ok('状态快照可纯 JSON 序列化', encoded.length > 0 && JSON.parse(encoded).schema === 1);
    ok('状态快照记录 RNG/牌区/执行状态', Number.isInteger(snap.rngState) && Array.isArray(snap.deck) && snap.players.length === 2 && snap.execution.state === 'idle');
    ...
    await pending;
    eq('决策完成恢复空闲执行状态', g.execution.state, 'idle');
  }
