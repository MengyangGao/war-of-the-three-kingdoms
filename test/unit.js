...
async function main() {
  ...
  /* 26. structured events and serializable state boundary */
  {
    const g = mkGame(['caocao', 'zhangliao']);
    const src = g.players[0], tgt = g.players[1];
    src.hand = []; tgt.hand = []; tgt.hp = 4;
    await g.damage({ source: src, target: tgt, amount: 1, element: 'normal' });
    const damageEvent = g.log.find(e => e.action === 'damage');
    ok('伤害日志包含结构化参与者', !!damageEvent && damageEvent.actorId === src.id && damageEvent.targetIds[0] === tgt.id && damageEvent.participantIds.length === 2);
    ok('所有日志使用统一事件信封', g.log.every(e =>
      e.schema === 2 && typeof e.eventId === 'string' && typeof e.eventType === 'string' &&
      Number.isInteger(e.turn) && Array.isArray(e.targetIds) && Array.isArray(e.participantIds)
    ));

    const snap = g.snapshot();
    const encoded = JSON.stringify(snap);
    ok('状态快照可纯 JSON 序列化', encoded.length > 0 && JSON.parse(encoded).schema === 2);
    ok('状态快照记录 RNG/牌区/执行状态', Number.isInteger(snap.rngState) && Array.isArray(snap.deck) && snap.players.length === 2 && snap.execution.state === 'notStarted');

    let releaseDecision;
    src.agent = { decide: () => new Promise(resolve => { releaseDecision = resolve; }) };
    const pending = g.ask(src, { type: 'confirm', reason: 'snapshot-test' });
    const waiting = g.snapshot();
    ok('等待决策可序列化为显式执行状态', waiting.execution.state === 'waitingForDecision' && waiting.execution.playerId === src.id && waiting.execution.request.type === 'confirm');
    releaseDecision({ yes: true });
    await pending;
    eq('决策完成恢复先前执行状态', g.execution.state, 'notStarted');

    const original = new SGS.Game({ numPlayers: 5, seed: 177, throwErrors: true, maxTurns: 800 });
    original.setup({ humanSeat: -1 });
    original.assignGenerals(['caocao', 'guanyu', 'sunquan', 'huatuo', 'lvbu']);
    original.players.forEach(p => { p.agent = SGS.AIAgent; });
    original.dealInitialHands();
    let checkpoint = null;
    original.checkpointCallback = value => { if (!checkpoint) checkpoint = value; };
    await original.start();
    ok('回合边界生成可恢复检查点', !!checkpoint && checkpoint.resumable && checkpoint.execution.state === 'readyTurn');
    const restored = SGS.Game.restore(JSON.parse(JSON.stringify(checkpoint)), { throwErrors: true });
    await restored.resume();
    eq('检查点恢复后胜方保持确定性', restored.winners, original.winners);
    eq('检查点恢复后回合数保持确定性', restored.turnCount, original.turnCount);
    const finalShape = game => JSON.stringify({
      deck: game.deck.map(c => c.id), discard: game.discard.map(c => c.id),
      players: game.players.map(p => ({
        hp: p.hp, alive: p.alive, hand: p.hand.map(c => c.id),
        equips: Object.keys(p.equips).map(k => p.equips[k] && p.equips[k].id),
        judge: p.judgeZone.map(c => c.id)
      }))
    });
    eq('检查点恢复后最终牌区完全一致', finalShape(restored), finalShape(original));

    let rejected = false;
    try { SGS.Game.restore(waiting); } catch (e) { rejected = /non-resumable/.test(e.message); }
    ok('拒绝恢复结算中的诊断快照', rejected);
  }

  /* 27. cancellable unified timeline */
  {
    let cancelled = false, fired = false;
    SGS.Timeline.after('unit-animation', 10000, () => { fired = true; }, () => { cancelled = true; });
    eq('时间线登记命名任务', SGS.Timeline.pending('unit-animation'), 1);
    eq('时间线按 scope 取消任务', SGS.Timeline.cancelScope('unit-animation'), 1);
    ok('取消任务释放回调且不会执行正文', cancelled && !fired && SGS.Timeline.pending('unit-animation') === 0);
    const delayed = SGS.Timeline.delay('unit-delay', 10000);
    SGS.Timeline.cancelScope('unit-delay');
    const delayedResult = await delayed;
    ok('取消时间线延迟会释放等待中的 Promise', delayedResult.cancelled === true);
  }

  /* 28. per-table balance profile and traitor endgame safety */
  {
    const g = new SGS.Game({ numPlayers: 8, seed: 91, throwErrors: true });
    g.setup({ humanSeat: -1 });
    g.assignGenerals(['caocao', 'guanyu', 'sunquan', 'huatuo', 'lvbu', 'zhaoyun', 'zhouyu', 'simayi']);
    g.players.forEach(p => { p.agent = SGS.AIAgent; });
    g.dealInitialHands();
    const lord = g.players.find(p => p.role === 'lord');
    const loyalists = g.players.filter(p => p.role === 'loyalist');
    const traitor = g.players.find(p => p.role === 'traitor');
    ok('8人平衡档给主忠各1张开局补偿', lord.hand.length === 5 && loyalists.every(p => p.hand.length === 5));
    eq('8人平衡档给内奸2张开局补偿', traitor.hand.length, 6);

    lord.hp = 1;
    ok('反贼存活时内奸不会对1血主公做致命攻击', !SGS.ai.isFoe(g, traitor, lord));
    lord.hp = 2;
    ok('内奸可压制健康主公以平衡桌面', SGS.ai.isFoe(g, traitor, lord) || SGS.ai.foes(g, traitor).length > 0);
  }

  /* 29. player-facing target legality explanations */
  {
    const g = mkGame(['caocao', 'guanyu', 'sunquan', 'huatuo', 'lvbu']);
    const src = g.players[0], near = g.players[1], far = g.players[2];
    const sha = C('sha', 'spade', 7);
    src.hand = [sha];
    const shaOption = SGS.playOptions(g, src).find(o => o.card === sha);
    ok('目标解释识别合法杀目标', SGS.TargetGuide.reason(g, src, near, shaOption).indexOf('可选') === 0);
    ok('目标解释给出距离与攻击范围', /距离 2.*范围 1/.test(SGS.TargetGuide.reason(g, src, far, shaOption)));

    near.hand = []; near.equips = { weapon: null, armor: null, offhorse: null, defhorse: null };
    const steal = { kind: 'card', name: 'shunshouqianyang', candidates: [] };
    eq('目标解释说明无牌可顺手', SGS.TargetGuide.reason(g, src, near, steal), '没有可获得的牌');
    far.hand = [C('shan', 'heart', 2)];
    eq('目标解释说明锦囊距离限制', SGS.TargetGuide.reason(g, src, far, steal), '距离超过 1');
  }
