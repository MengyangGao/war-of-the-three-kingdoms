...
async function runGame(SGS, numPlayers, seed, opts) {
  ...
  const invalidEvents = game.log.filter(entry =>
    entry.schema !== 2 ||
...
async function main() {
  ...
  let errors = 0, unconserved = 0, unfinished = 0, eventIssues = 0;
  let totalTurns = 0;
  const errSamples = [];
  ...
  console.log('Event issues :', eventIssues);
  ...
  const ok = errors === 0 && unconserved === 0 && eventIssues === 0;
  console.log('\nRESULT:', ok ? 'PASS' : 'FAIL');
  process.exit(ok ? 0 : 1);
}

module.exports = { loadSandbox, totalCards, pickGenerals, runGame };
