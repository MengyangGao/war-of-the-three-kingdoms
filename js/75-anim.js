...
(function (root) {
  ...
  function delay(ms) { return U.delay(ms); }
    ...
    return new Promise(function (resolve) {
      ...
      setTimeout(function () {
        if (!keep) {
          ...
          setTimeout(function () { fly.el.remove(); resolve(); }, 160);
        } else resolve();
      }, ms);
    });
  }
  ...
  Anim.useCard = function (data) {
    ...
    return animateFly(fly, dur('playCard'), true).then(function () {
      ...
      setTimeout(function () { fly.el.remove(); }, 260);
  ...
  Anim.aoe = function (data) {
    ...
    setTimeout(function () { ring.remove(); }, dur('aoe'));
