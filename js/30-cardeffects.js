...
(function (root) {
    ...
    while (player.alive) {
      ...
      if (opt.special === 'bagua') {
        ...
        if (jc && SGS.isRed(jc.suit)) {
          game.skillMsg(player, 'bagua', player.name + ' 八卦阵判定 ' + SGS.cardLabel(jc) + '，红色，视为“闪”。');
          return { special: 'bagua', virtual: true };
        }
        game.skillMsg(player, 'bagua', player.name + ' 八卦阵判定 ' + (jc ? SGS.cardLabel(jc) : '') + '，失败。');
        continue;
      }
      if (opt.special === 'zhangba') {
        ...
        game.skillMsg(player, 'zhangbashemao', player.name + ' 用丈八蛇矛将两张牌当“杀”。');
      ...
      if (opt.special === 'hujia' || opt.special === 'jijiang') {
        ...
        for (var ai2 = 0; ai2 < allies.length; ai2++) {
          ...
          if (ar) {
            ...
            game.skillMsg(ally, opt.special, ally.name + ' 响应' + (opt.special === 'hujia' ? '护驾' : '激将') + '，替 ' + player.name + ' 打出' + (need2 === 'shan' ? '“闪”' : '“杀”'), [player]);
            helped = true; break;
          }
        }
    ...
    if (card.name === 'sha') {
      ...
      try {
        ...
        game.msg(player.name + ' 对 ' + targets.map(function (t) { return t.name; }).join('、') +
          ...
          {
            kind: 'basic', player: player.id, actorId: player.id,
            targetIds: targets.map(function (target) { return target.id; }),
            action: 'useCard', cardName: card.name, element: card.element
          });
  ...
  SGS.doSha = async function (game, source, targets, card) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if (ctx.transferTo && ctx.transferTo.alive) {
        game.skillMsg(target, 'liuli', target.name + ' 发动流离，将“杀”转移给 ' + ctx.transferTo.name, [source, ctx.transferTo]);
      ...
      var provided = 0;
      if (ctx.undodgeable) {
        game.skillMsg(source, 'tieqi', target.name + ' 无法使用“闪”（铁骑）。', [target]);
      } else {
      ...
      if (source.equips.weapon && source.equips.weapon.name === 'hanbingjian' && discardable > 0) {
        ...
        if (useIce && useIce.yes) {
          ...
          game.msg(source.name + ' 发动寒冰剑，弃置了 ' + target.name + ' 的牌。', { kind: 'equip', actorId: source.id, targetIds: [target.id], action: 'equipEffect', cardName: 'hanbingjian' });
          continue;
        }
      }
  ...
  async function extraShaAfterDodge(game, source, target, card) {
    ...
    game.msg(source.name + ' 借青龙偃月刀再次“杀” ' + target.name, { kind: 'equip', actorId: source.id, targetIds: [target.id], action: 'equipEffect', cardName: 'qinglongyanyuedao' });
  ...
  async function guanshifuForce(game, source, target, card, ctx) {
    ...
    game.msg(source.name + ' 发动贯石斧，强制造成伤害。', { kind: 'equip', actorId: source.id, targetIds: [target.id], action: 'equipEffect', cardName: 'guanshifu' });
  ...
  async function cixiongEffect(game, source, target) {
    ...
    if (key === 'discard' && target.hand.length) {
      ...
      game.msg('雌雄双股剑：' + target.name + ' 弃置一张手牌。', { kind: 'equip', actorId: source.id, targetIds: [target.id], action: 'equipEffect', cardName: 'cixiongjian' });
    } else {
      await game.drawCards(source, 1, { reason: 'cixiong' });
      game.msg('雌雄双股剑：' + source.name + ' 摸一张牌。', { kind: 'equip', actorId: source.id, targetIds: [target.id], action: 'equipEffect', cardName: 'cixiongjian' });
    }
  }
  ...
  async function qilinEffect(game, source, target) {
    ...
    game.msg('麒麟弓：' + source.name + ' 弃置了 ' + target.name + ' 的坐骑。', { kind: 'equip', actorId: source.id, targetIds: [target.id], action: 'equipEffect', cardName: 'qilingong' });
  }
  ...
  SGS.useTrick = async function (game, source, card, targets, action) {
    ...
    try {
      ...
      game.msg(source.name + ' 使用【' + card.cn + '】' + (targets.length ? ('，目标：' + targets.map(function (t) { return t.name; }).join('、')) : ''), {
        kind: 'trick', player: source.id, actorId: source.id,
        targetIds: targets.map(function (target) { return target.id; }),
        action: 'useCard', cardName: card.name
      });
