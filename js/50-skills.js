  ...
  var S = SGS.SKILLS = {
    ...
    jianxiong: {
      ...
      triggers: {
        afterDamage: async function (game, me, ev) {
          ...
          game.skillMsg(me, 'jianxiong', me.name + ' 发动【奸雄】，获得造成伤害的牌。', ev.source ? [ev.source] : []);
        }
      }
    },
      ...
      triggers: {
        afterDamage: async function (game, me, ev) {
          ...
          if (card) {
            ...
            game.skillMsg(me, 'fankui', me.name + ' 发动【反馈】。', [src]);
          }
        }
      }
    },
        ...
        judgeCard: async function (game, me, ev) {
          ...
          if (res && res.yes) {
            ...
            if (c) {
              ...
              ev.forceCard = c;
              ev._guicaiDone = true;
              game.skillMsg(me, 'guicai', me.name + ' 发动【鬼才】改判。', [ev.player]);
    ...
    tuxi: {
      ...
      triggers: {
        drawPhaseNum: async function (game, me, ev) {
          ...
          game.skillMsg(me, 'tuxi', me.name + ' 发动【突袭】。', ps);
        }
      }
    },
    luoyi: {
      ...
      triggers: {
        drawPhaseNum: async function (game, me, ev) {
          ...
          if (res && res.yes) { ev.num -= 1; me.flags.luoyi = true; game.skillMsg(me, 'luoyi', me.name + ' 发动【裸衣】。'); }
        },
    ...
    tiandu: {
      ...
      triggers: {
        judgeResult: async function (game, me, ev) {
          ...
          game.skillMsg(me, 'tiandu', me.name + ' 发动【天妒】获得判定牌 ' + SGS.cardLabel(ev.card));
        }
      }
    },
    yiji: {
      ...
      triggers: {
        afterDamage: async function (game, me, ev) {
          ...
          game.skillMsg(me, 'yiji', me.name + ' 发动【遗计】。', ev.source ? [ev.source] : []);
        }
      }
    },
    luoshen: {
      ...
      triggers: {
        turnStart: async function (game, me, ev) {
          ...
          if (gained) game.skillMsg(me, 'luoshen', me.name + ' 发动【洛神】获得 ' + gained + ' 张牌。');
        }
      }
    },
    qingguo: {
    ...
    shensu: {
      ...
      onActivate: async function (game, me, action) {
        ...
        game.skillMsg(me, 'shensu', me.name + ' 发动【神速】。', [t]);
    ...
    rende: {
      ...
      onActivate: async function (game, me, action) {
        ...
        game.skillMsg(me, 'rende', me.name + ' 发动【仁德】，将 ' + give.length + ' 张牌交给 ' + t.name, [t], { amount: give.length });
    ...
    guanxing: {
      ...
      triggers: {
        turnStart: async function (game, me, ev) {
          ...
          game.skillMsg(me, 'guanxing', me.name + ' 发动【观星】，整理了牌堆顶' + (newBottom.length ? ('（' + newBottom.length + ' 张置底）') : '') + '。', [], { amount: top.length, bottomCount: newBottom.length });
        }
      }
    },
    kongcheng: {
    ...
    tieqi: {
      ...
      triggers: {
        shaHit: async function (game, me, ev) {
          ...
          if (jc && SGS.isRed(jc.suit)) { ev.undodgeable = true; game.skillMsg(me, 'tieqi', me.name + ' 铁骑判定红色，此“杀”不可被闪避。', ev.target ? [ev.target] : []); }
        }
      }
    },
    jizhi: {
      ...
      triggers: {
        afterUseTrick: async function (game, me, ev) {
          ...
          game.skillMsg(me, 'jizhi', me.name + ' 发动【集智】摸一张牌。');
        }
      }
    },
    qicai: {
    ...
    zhiheng: {
      ...
      onActivate: async function (game, me, action) {
        ...
        game.skillMsg(me, 'zhiheng', me.name + ' 发动【制衡】，换了 ' + toss.length + ' 张牌。', [], { amount: toss.length });
      },
      aiPlay: function (game, me, o, mk, ai) {
    ...
    keji: {
      ...
      triggers: {
        phaseStart: async function (game, me, ev) {
          ...
          if (!me.flags.shaThisTurn) { ev.skip.flag = true; game.skillMsg(me, 'keji', me.name + ' 发动【克己】跳过弃牌阶段。'); }
        }
      }
    },
    kurou: {
      ...
      onActivate: async function (game, me) {
        game.skillMsg(me, 'kurou', me.name + ' 发动【苦肉】。');
    ...
    fanjian: {
      ...
      onActivate: async function (game, me, action) {
        ...
        game.skillMsg(me, 'fanjian', me.name + ' 发动【反间】，' + t.name + ' 选择 ' + SGS.SUITS[chosen].cn + '，获得并展示 ' + SGS.cardLabel(card), [t], { suit: chosen, cardName: card.name });
    ...
    lianying: {
      ...
      triggers: {
        loseCardZone: async function (game, me, ev) {
          if (ev.player === me && ev.area === 'hand' && me.hand.length === 0) { await game.drawCards(me, 1, { reason: 'lianying' }); game.skillMsg(me, 'lianying', me.name + ' 发动【连营】。'); }
        }
      }
    },
    jieyin: {
      ...
      onActivate: async function (game, me, action) {
        ...
        game.skillMsg(me, 'jieyin', me.name + ' 发动【结姻】。', [t]);
      },
      aiPlay: function (game, me, o, mk, ai) {
    ...
    xiaoji: {
      ...
      triggers: {
        loseCardZone: async function (game, me, ev) {
          if (ev.player === me && ev.area === 'equip') { await game.drawCards(me, 2, { reason: 'xiaoji' }); game.skillMsg(me, 'xiaoji', me.name + ' 发动【枭姬】。'); }
        }
      }
    },
    ...
    qingnang: {
      ...
      onActivate: async function (game, me, action) {
        ...
        game.skillMsg(me, 'qingnang', me.name + ' 发动【青囊】为 ' + t.name + ' 回复体力。', [t]);
      },
      aiPlay: function (game, me, o, mk, ai) {
    ...
    lihun: {
      ...
      onActivate: async function (game, me, action) {
        ...
        game.skillMsg(me, 'lihun', me.name + ' 发动【离间】，令 ' + ts[0].name + ' 与 ' + ts[1].name + ' 决斗。', ts.slice(0, 2));
    ...
    biyue: {
      ...
      triggers: {
        phaseEnd: async function (game, me, ev) {
          if (ev.player === me && ev.phase === 'end') { await game.drawCards(me, 1, { reason: 'biyue' }); game.skillMsg(me, 'biyue', me.name + ' 发动【闭月】。'); }
        }
      }
    },
    xiaoyong: {
      ...
      triggers: {
        afterDamage: async function (game, me, ev) {
          ...
          game.skillMsg(me, 'xiaoyong', me.name + ' 发动【骁勇】摸 ' + n + ' 张牌。', ev.source ? [ev.source] : [], { amount: n });
        }
      }
    }
  };
