...
(function (root) {
  ...
  function Game(opts) {
    ...
    this.execution = { state: 'notStarted', currentId: null, phaseIndex: 0, resumable: true, request: null };
    this.checkpointCallback = opts.checkpointCallback || null;
  }
  ...
  /* Versioned snapshots and restoration live in js/41-checkpoint.js. */
  ...
  Game.prototype.emitLog = function (entry) {
    entry = entry || {};
    if (!entry.schema) entry.schema = 2;
    if (!entry.eventId) entry.eventId = 'e' + (++this.eventSequence);
    if (!entry.eventType) entry.eventType = entry.action || entry.kind || 'message';
    if (entry.turn == null) entry.turn = this.turnCount;
    if (entry.phase == null) entry.phase = this.phase;
    if (!Array.isArray(entry.targetIds)) entry.targetIds = [];
    var participants = [];
  ...
  Game.prototype.skillMsg = function (owner, skillName, text, targets, data) {
    var entry = data || {};
    entry.kind = 'skill';
    entry.eventType = 'skill';
    entry.action = 'skill';
    entry.actorId = owner && owner.id;
    entry.player = owner && owner.id;
    entry.targetIds = (targets || []).filter(Boolean).map(function (target) { return target.id; });
  ...
  Game.prototype.uiPause = async function (ms) {
    ...
    var wait = (ms == null ? 260 : ms) * (SGS.PACE == null ? 1 : SGS.PACE);
    return SGS.Timeline ? SGS.Timeline.delay('pacing', wait) : SGS.util.delay(wait);
  };
  ...
  // Original per-table opening balance. Larger tables amplify coordinated
  // rebel focus-fire and leave the lone traitor without rescue partners, so
  // those roles receive explicit, testable opening-card compensation.
  var OPENING_BONUS = SGS.OPENING_BONUS = {
    3: {},
    4: { lord: 1 },
    5: {},
    6: { lord: 1, loyalist: 1, traitor: 1 },
    7: { traitor: 2 },
    8: { lord: 1, loyalist: 1, traitor: 2 }
  };
  ...
  Game.prototype.dealInitialHands = function () {
    ...
    for (var i = 0; i < this.players.length; i++) {
      var p = this.players[i];
      var bonus = (OPENING_BONUS[this.numPlayers] && OPENING_BONUS[this.numPlayers][p.role]) || 0;
      var cards = this.deck.splice(0, 4 + bonus);
      ...
      for (var i = 0; i < order.length; i++) {
        ...
        if (res && res.card) {
          ...
          if (player.hasSkill('jiuyuan') && savior !== player && savior.nation === 'wu' && this.current !== savior && player.hp > 0) {
            this.skillMsg(player, 'jiuyuan', player.name + ' 发动【救援】，' + savior.name + ' 的桃额外回复1点。', [savior]);
  ...
  Game.prototype.start = async function () {
    ...
    this.phase = null;
    this.execution = { state: 'readyTurn', currentId: this.current.id, phaseIndex: 0, resumable: true, request: null };
    return this.continueFromCheckpoint();
  };

  Game.prototype.resume = async function () {
    if (!this.started) throw new Error('Cannot resume a game that has not started');
    if (this.finished) return this.winners;
    if (!this.execution || this.execution.state !== 'readyTurn' || !this.execution.resumable) {
      throw new Error('Game can only resume from a readyTurn checkpoint');
    }
    return this.continueFromCheckpoint();
  };

  Game.prototype.continueFromCheckpoint = async function () {
    while (!this.finished && this.turnCount < this.maxTurns) {
      if (!this.current || !this.current.alive) this.current = this.nextPlayer(this.current);
      if (!this.current) break;
      this.phase = null;
      this.execution = { state: 'readyTurn', currentId: this.current.id, phaseIndex: 0, resumable: true, request: null };
      if (this.checkpointCallback && this.snapshot) {
        try { this.checkpointCallback(this.snapshot()); }
        catch (checkpointError) {
          this.msg('检查点保存失败：' + checkpointError.message, { kind: 'error', eventType: 'checkpointError' });
          if (this.opts.throwErrors) throw checkpointError;
        }
      }
      this.execution = { state: 'resolvingTurn', currentId: this.current.id, phaseIndex: 0, resumable: false, request: null };
    ...
    this.execution = { state: 'finished', currentId: this.current && this.current.id, phaseIndex: 0, resumable: true, request: null };
    return this.winners;
  };
  ...
  Game.prototype.runPhase = async function (player, phase) {
    var previousExecution = this.execution;
    this.execution = {
      state: 'resolvingPhase', currentId: player.id, playerId: player.id,
      phase: phase, phaseIndex: SGS.PHASES.indexOf(phase), resumable: false, request: null
    };
    try {
      var skip = { flag: false };
