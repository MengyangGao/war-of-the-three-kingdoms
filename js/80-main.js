...
(function (root) {
  ...
  function readCheckpoint() {
    try {
      ...
      if (value.schema !== SGS.CHECKPOINT_SCHEMA || !value.resumable || !value.started || value.finished) return null;
      return value;
    } catch (e) { return null; }
  }

  function saveCheckpoint(value) {
    try { localStorage.setItem(CHECKPOINT_KEY, JSON.stringify(value)); } catch (e) {}
  }

  function clearCheckpoint() {
    try { localStorage.removeItem(CHECKPOINT_KEY); } catch (e) {}
    var button = $('continueBtn');
  ...
  function showContinueIfAvailable() {
    var button = $('continueBtn');
    if (button) button.classList.toggle('hidden', !readCheckpoint());
  }
