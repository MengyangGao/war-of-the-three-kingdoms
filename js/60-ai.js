...
(function (root) {
  ...
  function combatPower(players) {
    return players.reduce(function (sum, p) {
      return sum + Math.max(0, p.hp) * 2 + p.hand.length + (p.equips.weapon ? 1 : 0) + (p.equips.armor ? 1 : 0);
    }, 0);
  }

  function traitorTargetCamp(game) {
    var alive = game.alivePlayers();
    var rebels = alive.filter(function (p) { return p.role === 'rebel'; });
    if (!rebels.length) return 'court';
    var court = alive.filter(function (p) { return p.role === 'lord' || p.role === 'loyalist'; });
    return combatPower(rebels) >= combatPower(court) * 0.9 ? 'rebel' : 'court';
  }
  ...
  function foeByRole(game, me, other) {
    ...
    if (me.role === 'traitor') {
      ...
      var targetCamp = traitorTargetCamp(game);
      if (targetCamp === 'rebel') return other.role === 'rebel';
      // Keep the lord alive as the traitor's required final opponent; dismantle
      // loyalists first while the table still has three or more survivors.
      if (other.role === 'loyalist') return true;
      // With no loyalist left, pressure a healthy lord without ever making a
      // lethal attack that would hand the win directly to the rebels.
      if (other.role === 'lord') return other.hp > 1;
      return other.role === 'rebel' && game.players[0].hp <= 1;
    }
  ...
  function foeByAlign(game, me, other) {
    ...
    switch (me.role) {
      ...
      case 'traitor':
        if (game.aliveCount() <= 2) return true;
        if (traitorTargetCamp(game) === 'rebel') return !lord && a <= -0.6;
        if (lord) return other.hp > 1;
        return a >= 0.6 || (game.players[0].hp <= 1 && a <= -0.6);
  ...
  ai.rankFoe = function (game, me, foe) {
    ...
    if (isLord(game, foe)) score += me.role === 'rebel' || (me.role === 'traitor' && game.aliveCount() <= 2) ? 8 : 0;
  ...
  ai.scorePlayOption = function (game, player, o) {
    ...
    if (name === 'sha') {
      ...
      if (target.role === 'lord' && (player.role === 'rebel' || (player.role === 'traitor' && game.aliveCount() <= 2))) score += 2;
      // multi-target 方天画戟
      var targets = [target];
      if (o.maxTargets > 1) {
