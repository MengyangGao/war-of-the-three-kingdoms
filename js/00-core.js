...
(function (root) {
  ...
  SGS.RNG = function (seed, restoredState) {
    var s = restoredState == null ? ((seed >>> 0) || 0x9e3779b9) : (restoredState >>> 0);
    var fn = function () {
    ...
    fn.setState = function (next) {
      if (!Number.isInteger(next) || next < 0 || next > 0xffffffff) throw new RangeError('RNG state must be an unsigned 32-bit integer');
      s = next >>> 0;
      return fn;
    };
    return fn;
  ...
  U.uid.state = function () { return _id; };
  U.uid.setState = function (next) {
    if (!Number.isInteger(next) || next < 0) throw new RangeError('UID state must be a non-negative integer');
    _id = next;
  };
