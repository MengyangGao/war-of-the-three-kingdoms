...
- 目标不再靠试错：选牌后，不能选择的角色会直接说明距离、无牌、重复延时锦囊或技能保护等原因。
- 角色事件轨迹：每名角色最多保留最近五条攻击、受伤、救援、技能、计策与装备互动，并用箭头区分发起方与承受方。
- 即时速度控制：牌桌顶栏可随时切换慢速、标准、快速和极速；演出期间还可立即跳过当前动画。
- 安全续局：每个回合边界自动保存；续局卡片显示人数、武将、回合和保存时间，损坏存档会给出可见反馈。
...
js/78-guidance.js          操作步骤、选择摘要与目标合法性解释
js/80-main.js              大厅与游戏启动装配
test/                      结构、单元、模拟与浏览器测试
tools/audit_assets.py      Wikimedia Commons 许可审计
```

规则引擎与展示层的边界、状态不变量及扩展方式见 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)；本轮十项体验审查见 [`docs/UX_AUDIT.md`](docs/UX_AUDIT.md)，新版体验规格见 [`docs/REDESIGN.md`](docs/REDESIGN.md)，同类项目与标准调研见 [`docs/RESEARCH.md`](docs/RESEARCH.md)。

## 质量检查

```bash
npm run check              # 脚本顺序、数据引用、牌堆、画像和资产账本
npm test                   # 98 项断言、60 局模拟、3–8 人平衡矩阵、单文件校验
